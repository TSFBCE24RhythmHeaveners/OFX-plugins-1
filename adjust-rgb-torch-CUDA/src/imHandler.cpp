...
void imageCopy::toTensor(){
  // copy to tensor from direct memory access
  imTensor = torch::from_blob(imageVec.data(),
                              {getNumRows(), getNumCols(), getNumChans()});
  // make channels first
  imTensor = imTensor.permute({2, 0, 1});
}

void imageCopy::toVec(){
  // copy tensor to vector mem (vector much quicker to read elemenets)
  std::memcpy((void *) imageVec.data(),
              imTensor.data_ptr(),
              sizeof(torch::kFloat) * imTensor.numel());
}
...
// template <class T>
float imageCopy::getTensorPix(int row, int col, int chan){
  if (row>=n_rows || col>=n_cols){
    std::cout<<"Pixel out of bounds\n";
    return -1;
  }
  // super slow for some reason?????
  return imTensor[chan][row][col].item<float>();
}
